...
//    NSMutableArray* singles = [NSMutableArray arrayWithArray:annos];
	while(TRUE)
	{
...
//    NSMutableArray* singleAnnotationsMinusGrouped  = [NSMutableArray arrayWithArray:annos];
	...
	for(NSArray* group in groupings)
	{
        ...
        ((GroupedAnnotation *)groupedAnnotation).categoryName = [self findCategoryNameOfGrouped:group];
        ((GroupedAnnotation *)groupedAnnotation).listAnnotations = group;
...
//    [singleAnnotationsMinusGrouped release];
}
-(NSString *)findCategoryNameOfGrouped:(NSArray *)group
{
    SingleAnnotation * spinMinimum =nil;
    SingleAnnotation *spinTemp =nil;
    CGFloat distanceMax =100000.0f;
    CGFloat distanceTemp;
    for(id<MKAnnotation> a1 in group)
    {
        if ([a1 class]==[SingleAnnotation class]) {
            spinTemp = (SingleAnnotation *)a1;
            distanceTemp = spinTemp.distance;
            if (distanceTemp <distanceMax) {
                distanceMax = distanceTemp;
                spinMinimum = spinTemp;
            }
        }
        
    }
    NSString *result = [NSString stringWithFormat:@"%@",spinMinimum.categoryName];
    return result;
